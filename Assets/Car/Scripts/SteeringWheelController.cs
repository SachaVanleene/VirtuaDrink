...
public class SteeringWheelController : MonoBehaviour
{
    ...
    private SteamVR_Controller.Device TrackedController;
    ...
    private float INERTIA = 0f; // 1-wheel never stops // 0 - wheel stops instantly
    ...
    void Start()
    {
        ...
        TrackedController = SteamVR_Controller.Input((int) trackedObj.index); 
    ...
    public void OnStick(SteamVR_Controller.Device TrackedController, GameObject HandOnSteering)
    {
        if (HandSticked != true)
        {
            CalculateOffset();
    ...
    void FixedUpdate()
    {
        ...
        WheelBase.transform.Rotate(new Vector3(0, 0, 1), 0.1f * outputAngle);
    ...
    {
        if (other.name == "Steering Wheel")
        {
            if (TrackedController.GetHairTriggerDown())
            {
                Debug.Log(gameObject.name + " Trigger Press");
                OnStick(TrackedController, Hand);
            }

            if (TrackedController.GetHairTriggerUp())
            {
